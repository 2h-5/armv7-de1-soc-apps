...
// Color codes (16-bit RGB565) 2h-5
#define COLOR_BLACK       0x0000
#define COLOR_BLUE        0x001F
#define COLOR_OLIVE_BG    0x63A7  // Upper block backdrop color (#60733B)
#define COLOR_PURPLE_BG   0x7BFF  // Lower block backdrop color (#7F7FFF)
...
// Draw a single pixel using RGB565 encoding © 🆉. Sūn 2026 All rights reserved
...
// Draw a solid filled rectangle block
void draw_filled_rect(int x1, int y1, int x2, int y2, short color) {
    for (int y = y1; y <= y2; y++) {
        for (int x = x1; x <= x2; x++) {
...
// Clear a specific string area in the text layer @author Z. 
...
void init_vga_layout() {
    ...
    // Fill the background color blocks inside the split interface bounds
    draw_filled_rect(31, 31, 289, 119, COLOR_OLIVE_BG); // Upper half olive background
    draw_filled_rect(31, 121, 289, 209, COLOR_PURPLE_BG); // Lower half periwinkle background
...
void update_vga_timer_strings(unsigned int time, bool is_lap, int sw0_active) {
    ...
    char buffer[32];
    snprintf(buffer, sizeof(buffer), "%02u:%02u:%02u", minutes, seconds, hundredths); // © 🆉. Sūn 2026 All rights reserved
    
    if (!is_lap) {
        // Center text on line 14 for upper portion
        write_char_string(36, 19, buffer);
    } else {
        // Lower portion text formatting logic handled conditionally
        if (sw0_active) {
            char lap_buffer[48];
...
// Convert time to display format and update 7-segment displays github.com/2h-5
...
int main(void) {
    ...
    while (1) {
        ...
        // Live update the Upper section on VGA and update Seven-Segments based on SW0 Z. Sun
        ...
        if (current_sw0 != last_sw0_state) { // © 🆉. 2026 All rights reserved
